...
class MainActivity : AppCompatActivity() {
    ...
    private lateinit var toolStatusTv: TextView
    ...
    private lateinit var lawSearchService: LawSearchService
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        toolStatusTv = findViewById(R.id.tool_status)
        ...
        lawSearchService = LawSearchService(applicationContext)
        lifecycleScope.launch(Dispatchers.IO) {
            val ready = lawSearchService.initialize()
            withContext(Dispatchers.Main) {
                toolStatusTv.text = if (ready) {
                    "오프라인 법령 DB 준비 완료"
                } else {
                    "오프라인 법령 DB 미탑재: assets/db/school_law.db 확인 필요"
                }
            }
        }
            ...
            contentResolver.openInputStream(uri)?.use {
                ...
                contentResolver.openInputStream(uri)?.use { input ->
                    ...
                    engine.setSystemPrompt(
                        "당신은 오프라인 한국 법령 도우미다. 제공된 근거 문맥을 벗어나 추측하지 말고, " +
                            "답변 끝에 반드시 근거 법령명을 제시한다."
                    )
        ...
        userInputEt.text.toString().also { userMsg ->
            if (userMsg.isEmpty()) {
                ...
                generationJob = lifecycleScope.launch(Dispatchers.Default) {
                    val retrieval = lawSearchService.let {
                        orchestrator.retrieve(userMsg, it)
                    }
                    var appendedReferences = false

                    withContext(Dispatchers.Main) {
                        toolStatusTv.text = orchestrator.renderToolStatus(retrieval)
                        ggufTv.text = orchestrator.renderLawViewer(retrieval)
                    }

                    val augmentedPrompt = orchestrator.buildAugmentedPrompt(userMsg, retrieval)

                    engine.sendUserPrompt(augmentedPrompt)
                        .onCompletion {
                            withContext(Dispatchers.Main) {
                                if (!appendedReferences && messages.isNotEmpty() && !messages.last().isUser) {
                                    val referenced = messages.last().copy(
                                        content = messages.last().content + orchestrator.renderReferences(retrieval)
                                    )
                                    messages[messages.lastIndex] = referenced
                                    messageAdapter.notifyItemChanged(messages.lastIndex)
                                    messagesRv.scrollToPosition(messages.lastIndex)
                                    appendedReferences = true
                                }
                                userInputEt.isEnabled = true
                                userActionFab.isEnabled = true
                            }
